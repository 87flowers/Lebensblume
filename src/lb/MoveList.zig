...
fn splatMaybePromoMoves(self: *MoveList, active_color: Color, from: Square, to_bb: Bitboard, comptime ptype: PieceType) void {
    const valid_normal_dests = switch (active_color) {
        inline else => |color| comptime switch (ptype) {
            .none => unreachable,
            .pawn, .lance => Bitboard.rankRelative(8, color),
            .knight => Bitboard.@"or"(Bitboard.rankRelative(7, color), Bitboard.rankRelative(8, color)),
            else => Bitboard{},
        }.invert(),
    };
